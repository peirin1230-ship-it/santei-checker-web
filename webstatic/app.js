...
function collapseContiguousByteRanges(chunks) {
  ...
  for (let x, y, i2 = 0, j2 = 0, n = chunks.length; ++i2 < n; ) {
    x = result[j2];
    y = chunks[i2];
...
var _setStructMapValue = (o, v2) => (set, c, f, _2) => c && set(c, o, v2.get(f.name));
var _setStructObjectValue = (o, v2) => (set, c, f, _2) => c && set(c, o, v2[f.name]);
var setStruct = (data, index, value) => {
  ...
  data.type.children.forEach((f, i2) => set(childSetters[i2], data.children[i2], f, i2));
};
...
var Table = class _Table {
    ...
    const unwrap = (x) => {
      if (x) {
        if (x instanceof RecordBatch2) {
          ...
          const batchSchema = schema !== null && schema !== void 0 ? schema : new Schema2(keys.map((k, i2) => new Field2(String(k), vecs[i2].type, vecs[i2].nullable)));
   ...
   */
  select(columnNames) {
    const nameToIndex = this.schema.fields.reduce((m2, f, i2) => m2.set(f.name, i2), /* @__PURE__ */ new Map());
...
function decodeFieldNodes(batch) {
  const nodes = [];
  for (let f, i2 = -1, j2 = -1, n = batch.nodesLength(); ++i2 < n; ) {
    if (f = batch.nodes(i2)) {
...
function decodeSchemaFields(schema, dictionaries) {
  const fields = [];
  for (let f, i2 = -1, j2 = -1, n = schema.fieldsLength(); ++i2 < n; ) {
...
function decodeFieldChildren(field, dictionaries) {
  const children = [];
  for (let f, i2 = -1, j2 = -1, n = field.childrenLength(); ++i2 < n; ) {
...
function encodeSchema(b2, schema) {
  const fieldOffsets = schema.fields.map((f) => Field2.encode(b2, f));
  Schema.startFieldsVector(b2, fieldOffsets.length);
  const fieldsVectorOffset = Schema.createFieldsVector(b2, fieldOffsets);
  const metadataOffset = !(schema.metadata && schema.metadata.size > 0) ? -1 : Schema.createCustomMetadataVector(b2, [...schema.metadata].map(([k, v2]) => {
...
function encodeField(b2, field) {
  ...
  const childOffsets = (type.children || []).map((f) => Field2.encode(b2, f));
  const childrenVectorOffset = Field.createChildrenVector(b2, childOffsets);
  const metadataOffset = !(field.metadata && field.metadata.size > 0) ? -1 : Field.createCustomMetadataVector(b2, [...field.metadata].map(([k, v2]) => {
    const key = b2.createString(`${k}`);
    const val = b2.createString(`${v2}`);
    KeyValue.startKeyValue(b2);
    KeyValue.addKey(b2, key);
...
// webstatic/vendor/duckdb-browser.mjs
var q = Object.create;
...
async function initDb(zipBuf) {
  $2("loading").textContent = "\u30C7\u30FC\u30BF\u30D9\u30FC\u30B9\u3092\u521D\u671F\u5316\u4E2D\u2026";
  ...
  const manifest = zip.files["manifest.json"] ? JSON.parse(await zip.files["manifest.json"].async("string")) : {};
  ckEdition = (await rows(
    "SELECT max(edition) AS e FROM checkmaster_iy_tekio"
  ))[0]?.e ?? null;
  $2("dataInfo").textContent = `(\u30C7\u30FC\u30BF\u751F\u6210\u65E5: ${manifest.generated ?? "\u4E0D\u660E"} / \u30C1\u30A7\u30C3\u30AF\u30DE\u30B9\u30BF: ${ckEdition ?? "\u306A\u3057"})`;
  $2("setup").style.display = "none";
  $2("app").style.display = "block";
}
...
var NOT_LISTED = "\u30C6\u30FC\u30D6\u30EB\u4E0A\u306E\u80CC\u53CD\u30FB\u5305\u62EC\u306E\u53CE\u8F09\u306A\u3057(\u672A\u53CE\u8F09\u30D1\u30BF\u30FC\u30F3\u306E\u53EF\u80FD\u6027\u3042\u308A)";
var UNLISTED_RULES_NOTE = "\u203B\u96FB\u5B50\u70B9\u6570\u8868\u306B\u306F\u610F\u56F3\u7684\u306B\u53CE\u8F09\u3055\u308C\u3066\u3044\u306A\u3044\u30EB\u30FC\u30EB\u304C\u3042\u308A\u307E\u3059(3\u9805\u76EE\u4EE5\u4E0A\u5B9F\u65BD\u6642\u306E\u300C\u4E3B\u305F\u308B\u3082\u306E2\u3064\u306B\u9650\u308A\u7B97\u5B9A\u300D\u7B49\u306E\u591A\u5BFE\u591A\u30EB\u30FC\u30EB\u3001\u50B7\u75C5\u540D\u30FB\u90E8\u4F4D\u7B49\u306E\u8981\u4EF6\u304C\u9650\u5B9A\u3055\u308C\u305F\u80CC\u53CD\u30012\u9805\u76EE\u306E\u7B97\u5B9A\u306B\u3088\u308A\u5225\u306E1\u9805\u76EE\u304C\u80CC\u53CD\u3068\u306A\u308B\u30B1\u30FC\u30B9\u3001\u8907\u6570\u8981\u4EF6\u306E\u80CC\u53CD\u3001\u88AB\u5305\u62EC\u9805\u76EE\u304C\u660E\u8A18\u3055\u308C\u3066\u3044\u306A\u3044\u5305\u62EC \u7B49)\u3002\u30C6\u30FC\u30D6\u30EB\u306B\u8A72\u5F53\u304C\u306A\u3044\u3053\u3068\u306F\u4F75\u7B97\u5B9A\u53EF\u3092\u610F\u5473\u3057\u307E\u305B\u3093\u3002";
...
var FW = "\uFF10\uFF11\uFF12\uFF13\uFF14\uFF15\uFF16\uFF17\uFF18\uFF19\uFF21\uFF22\uFF23\uFF24\uFF25\uFF26\uFF27\uFF28\uFF29\uFF2A\uFF2B\uFF2C\uFF2D\uFF2E\uFF2F\uFF30\uFF31\uFF32\uFF33\uFF34\uFF35\uFF36\uFF37\uFF38\uFF39\uFF3A\uFF0D";
var HW = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-";
var toHw = (s) => [...s].map((ch) => {
  const i2 = FW.indexOf(ch);
  return i2 >= 0 ? HW[i2] : ch;
}).join("");
var HAIHAN_TABLES = [
  ["haihan_day", "1\u65E5\u306B\u3064\u304D"],
  ["haihan_month", "\u540C\u4E00\u6708\u5185"],
  ["haihan_simultaneous", "\u540C\u6642"],
  ["haihan_week", "1\u9031\u9593\u306B\u3064\u304D"]
];
var HAIHAN_KUBUN = { "1": "\u30B3\u30FC\u30C9\u2460\u5074\u3092\u7B97\u5B9A", "2": "\u30B3\u30FC\u30C9\u2461\u5074\u3092\u7B97\u5B9A", "3": "\u3044\u305A\u308C\u304B\u4E00\u65B9\u3092\u7B97\u5B9A" };
var HOKATSU_TANI = {
  "01": "1\u65E5\u306B\u3064\u304D",
  "1": "1\u65E5\u306B\u3064\u304D",
  "02": "\u540C\u4E00\u6708\u5185",
...
async function resolveRef(ref, source) {
  ref = ref.trim();
  let hit = await rows(
    "SELECT filename FROM doc_ref WHERE source=? AND ref=?",
    [source, ref]
  );
  if (hit.length) return hit[0].filename;
  const m2 = ref.match(/^([A-Z])(\d{3})(.*)$/);
  if (m2 && m2[3] && !m2[3].startsWith("-")) {
    hit = await rows(
      "SELECT filename FROM doc_ref WHERE source=? AND ref=?",
      [source, m2[1] + m2[2]]
    );
...
var likeEsc = (s) => s.replace(/\\/g, "\\\\").replace(/%/g, "\\%").replace(/_/g, "\\_");
...
async function lookupCode(code, edition, ym, monthStart) {
  ...
  if (r.length) {
    ...
    Object.assign(info, {
      found: true,
      name: x.shoryaku_kanji_meisho,
      kubun: x.kubun,
      nyugai: x.nyugai_tekiyo_kubun,
      tsusoku: x.tsusoku_nenrei,
      kagen: x.kagen_nenrei,
      jogen: x.jogen_nenrei,
      haishi: x.haishi_ymd,
      expired: !!x.haishi_ymd && x.haishi_ymd < monthStart
    });
    for (let i2 = 1; i2 <= 10; i2++) {
      const v2 = x[`shisetsu_kijun_code_${i2}`];
      if (v2 && v2 !== "0") info.shisetsu.push(v2);
    }
  }
  return info;
}
async function runCheck(codesInput, ym, ryoMap, nissuMap) {
  ...
  const hits = [];
  const notListed = [];
  for (let i2 = 0; i2 < shinryokoi.length; i2++) {
    for (let j2 = i2 + 1; j2 < shinryokoi.length; j2++) {
      const a = shinryokoi[i2], b2 = shinryokoi[j2];
      const pair = [];
      for (const [table, joken] of HAIHAN_TABLES) {
        const rws = await rows(`
          SELECT * FROM ${table} WHERE edition=?
            ...
            AND shinsetsu_ymd<=? AND haishi_ymd>=?`, [edition, a, b2, b2, a, me, ms]);
        if (!rws.length) continue;
        const kubuns = [...new Set(rws.map((r) => r.haihan_kubun))].sort();
        const tokurei = rws.some((r) => r.tokurei_joken === "1");
        const santei = [...new Set(rws.filter((r) => r.haihan_kubun === "1" || r.haihan_kubun === "2").map((r) => r.haihan_kubun === "1" ? r.shinryokoi_shoryaku_meisho_1 : r.shinryokoi_shoryaku_meisho_2))].sort();
        ...
        pair.push({
          ...
          tokurei,
          table: `${table}(${edition})`,
          evid: rws.map((r) => `\u2460${r.shinryokoi_code_1} ${r.shinryokoi_shoryaku_meisho_1} \xD7 \u2461${r.shinryokoi_code_2} ${r.shinryokoi_shoryaku_meisho_2} \u80CC\u53CD\u533A\u5206=${r.haihan_kubun}(${HAIHAN_KUBUN[r.haihan_kubun] ?? "\u4E0D\u660E"})/${tokureiLabel(r.tokurei_joken)}`),
          refs: [infos[a].kubun, infos[b2].kubun].filter(Boolean)
        });
      }
      for (const [p2, q_] of [[a, b2], [b2, a]]) {
        const hojo = await rows(`
          ...
          FROM hojo_master WHERE edition=? AND shinryokoi_code=?
            AND shinsetsu_ymd<=? AND haishi_ymd>=?`, [edition, p2, me, ms]);
        if (!hojo.length) continue;
        for (const i22 of [1, 2, 3]) {
          const g = hojo[0][`group_bango_${i22}`], tani = hojo[0][`hokatsu_tani_${i22}`];
          if (!g || g === "0") continue;
          const rws = await rows(`
          ...
          if (!rws.length) continue;
          const tokurei = rws.some((r) => r.tokurei_joken === "1");
          const refs = [infos[p2].kubun, infos[q_].kubun].filter(Boolean);
          if (g.length >= 4) {
          ...
          hits.push({
            kind: "hokatsu",
            title: "[\u5305\u62EC] \u5305\u62EC\u30FB\u88AB\u5305\u62EC\u30C6\u30FC\u30D6\u30EB",
            detail: `\u300C${infos[q_].name ?? q_}\u300D\u306F\u300C${infos[p2].name ?? p2}\u300D\u306B\u5305\u62EC\u3055\u308C\u7B97\u5B9A\u4E0D\u53EF(\u5305\u62EC\u5358\u4F4D: ${HOKATSU_TANI[tani] ?? `\u5358\u4F4D\u30B3\u30FC\u30C9${tani}(\u4E0D\u660E)`})/${tokurei ? "\u7279\u4F8B\u6761\u4EF6=1\u306E\u30D2\u30C3\u30C8\u3042\u308A(\u8981\u901A\u77E5\u78BA\u8A8D)" : "\u7279\u4F8B\u6761\u4EF6=0(\u7279\u5225\u306A\u6761\u4EF6\u306A\u3057)"}`,
            tokurei,
            table: `hokatsu(${edition})`,
            evid: rws.map((r) => `\u30B0\u30EB\u30FC\u30D7${g}(\u88DC\u52A9\u30DE\u30B9\u30BF\u30FC: ${p2} ${infos[p2].name ?? ""} \u5305\u62EC\u5358\u4F4D=${tani}) \u306B ${r.shinryokoi_code} ${r.shinryokoi_shoryaku_meisho} \u3092\u53CE\u8F09/${tokureiLabel(r.tokurei_joken)}`),
            refs
          });
          pair.push(hits[hits.length - 1]);
          hits.pop();
        }
      }
      if (pair.length) hits.push(...pair);
      else notListed.push([a, b2]);
    }
  }
  for (const base of shinryokoi) {
    ...
    if (!h2.length || h2[0].s === "000") continue;
    ...
    const kasan = await rows(`
      SELECT * FROM nyuin_kihonryo WHERE edition=? AND group_bango=?
        ...
        AND shinsetsu_ymd<=? AND haishi_ymd>=?`, [edition, h2[0].s, ...others, me, ms]);
    const by = {};
    ...
    for (const sk of Object.keys(by).sort()) {
      ...
      const detail = mem.length >= 2 ? `\u300C${infos[base].name ?? base}\u300D(\u30B0\u30EB\u30FC\u30D7${h2[0].s})\u306B\u5BFE\u3059\u308B\u52A0\u7B97\u306E\u3046\u3061\u3001\u540C\u4E00\u306E\u52A0\u7B97\u8B58\u5225${sk}\u304C\u4ED8\u3044\u305F\u300C${names}\u300D\u540C\u58EB\u306F\u4F75\u7B97\u5B9A\u4E0D\u53EF` : `\u300C${names}\u300D\u306F\u300C${infos[base].name ?? base}\u300D(\u30B0\u30EB\u30FC\u30D7${h2[0].s})\u306E\u5BFE\u8C61\u52A0\u7B97\u3068\u3057\u3066\u53CE\u8F09(\u52A0\u7B97\u8B58\u5225=${sk})`;
      hits.push({
        kind: "nyuin",
        title: "[\u5165\u9662\u57FA\u672C\u6599] \u5165\u9662\u57FA\u672C\u6599\u30C6\u30FC\u30D6\u30EB",
        detail,
        tokurei: false,
        table: `nyuin_kihonryo(${edition})`,
  ...
  if (ckEdition) {
    ...
    for (const drug of drugs) {
      const rep = {
        code: drug,
        name: infos[drug].name,
        kind: "\u533B\u85AC\u54C1",
        table: `checkmaster_iy_tekio(${ckEdition})`,
        listed: false,
        listedRows: 0,
        matches: [],
        unmatched: [],
    ...
    if (diseases.length) {
      for (const act of shinryokoi) {
        const rep = {
          ...
          name: infos[act].name,
          kind: "\u8A3A\u7642\u884C\u70BA",
          table: `checkmaster_si_shobyo(${ckEdition})`,
          listed: false,
          listedRows: 0,
          matches: [],
          unmatched: [],
  ...
  try {
    for (const c of codes) {
      ...
      if (info.kind === "\u8A3A\u7642\u884C\u70BA" && info.kubun) {
        const m2 = /^([A-Z])(\d{3})(.*)$/.exec(info.kubun);
        if (m2) {
          const base = m2[3] === "" || m2[3].startsWith("-") ? info.kubun : m2[1] + m2[2];
          rws = await rows(`
      ...
      for (const r of rws) {
        ...
        if (!blocks.length || blocks[blocks.length - 1].key !== key) {
          blocks.push({
            ...
            bessyo: r.bessyo,
            koban: r.koban,
            kubun_hyoki: r.kubun_hyoki,
            meisho: r.meisho,
            kisai_jiko: r.kisai_jiko,
            comments: []
          });
        }
...
var PROFILE_KUBUN_IMI = {
  "1": "\u81EA\u30B3\u30FC\u30C9\u5074\u3092\u7B97\u5B9A(\u76F8\u624B\u5074\u304C\u7B97\u5B9A\u4E0D\u53EF)",
  "2": "\u76F8\u624B\u5074\u3092\u7B97\u5B9A(\u81EA\u30B3\u30FC\u30C9\u304C\u7B97\u5B9A\u4E0D\u53EF)",
  "3": "\u3044\u305A\u308C\u304B\u4E00\u65B9\u3092\u7B97\u5B9A"
};
var tokureiLabel = (t) => t === "1" ? "\u7279\u4F8B\u6761\u4EF6=1(\u901A\u77E5\u306B\u7279\u5225\u306A\u6761\u4EF6\u3042\u308A\u30FB\u539F\u6587\u78BA\u8A8D\u5FC5\u9808)" : "\u7279\u4F8B\u6761\u4EF6=0(\u7279\u5225\u306A\u6761\u4EF6\u306A\u3057)";
...
var shinsaLink = (no, t) => `<a class="reflink" data-shinsa="${no}">No.${no}</a> ${esc(t)}`;
async function renderCheck(R2) {
  const out = [];
  const banners = [];
  out.push(`=== \u4E00\u6B21\u5224\u5B9A: \u8A3A\u7642\u5E74\u6708 ${esc(R2.ym)}(\u9069\u7528\u7248: ${esc(R2.edition)}) ===`);
  const tokureiHits = R2.hits.filter((h2) => h2.tokurei);
  if (tokureiHits.length) {
    banners.push("\u3010\u8981\u901A\u77E5\u78BA\u8A8D\u3011\u7279\u4F8B\u6761\u4EF6=1 \u306E\u30D2\u30C3\u30C8\u304C\u3042\u308A\u307E\u3059\u3002\u901A\u77E5\u539F\u6587\u306E\u78BA\u8A8D\u304C\u5FC5\u9808\u3067\u3059:<br>" + tokureiHits.map((h2) => `\u30FB${esc(h2.title)}: ${esc(h2.detail)}`).join("<br>"));
  }
  const kinkiReps = R2.tekiou.filter((t) => t.kinki.length);
  if (kinkiReps.length) {
    banners.push("\u3010\u7981\u5FCC\u306E\u7591\u3044\u3011\u30C1\u30A7\u30C3\u30AF\u30DE\u30B9\u30BF\u306E\u7981\u5FCC\u50B7\u75C5\u540D\u306B\u8A72\u5F53\u304C\u3042\u308A\u307E\u3059(\u6A5F\u68B0\u5224\u5B9A\u30FB\u8981\u539F\u6587\u78BA\u8A8D):<br>" + kinkiReps.flatMap((t) => t.kinki.map((k) => `\u30FB\u533B\u85AC\u54C1 ${esc(t.code)} ${esc(t.name ?? "")} \xD7 \u7981\u5FCC\u50B7\u75C5\u540D ${esc(k.kinki_shobyomei_code)} ${esc(R2.infos[k.kinki_shobyomei_code]?.name ?? "\u540D\u79F0\u4E0D\u660E")}(\u53C2\u7167\u7BC4\u56F2=${esc(k.sansho_hani)})`)).join("<br>"));
  }
  if (R2.heiyo.length) {
    banners.push("\u3010\u4F75\u7528\u7981\u5FCC\u306E\u7591\u3044\u3011\u30C1\u30A7\u30C3\u30AF\u30DE\u30B9\u30BF\u306E\u4F75\u7528\u7981\u5FCC\u306E\u7D44\u5408\u305B\u306B\u8A72\u5F53\u304C\u3042\u308A\u307E\u3059(\u6A5F\u68B0\u5224\u5B9A\u30FB\u8981\u6DFB\u4ED8\u6587\u66F8\u78BA\u8A8D):<br>" + R2.heiyo.map((h2) => `\u30FB${esc(h2.iyakuhin_code_l)} ${esc(R2.infos[h2.iyakuhin_code_l]?.name ?? "")} \xD7 ${esc(h2.iyakuhin_code_r)} ${esc(R2.infos[h2.iyakuhin_code_r]?.name ?? "")}(\u53C2\u7167\u7BC4\u56F2=${esc(h2.sansho_hani)})`).join("<br>"));
  }
  ...
  out.push("", "--- \u30B3\u30FC\u30C9\u60C5\u5831 ---");
  for (const c of R2.codes) {
    const i2 = R2.infos[c];
    if (i2.kind === "\u533B\u85AC\u54C1" || i2.kind === "\u50B7\u75C5\u540D") {
      out.push(i2.found ? `  ${esc(c)}: ${esc(i2.name)}(\u7A2E\u5225=${i2.kind} \u2192 \u9069\u5FDC\u75C5\u540D\u30C1\u30A7\u30C3\u30AF\u306E\u5BFE\u8C61)` : `  ${esc(c)}: \u7A2E\u5225=${i2.kind} \u2192 \u30DE\u30B9\u30BF\u30FC\u306B\u53CE\u8F09\u306A\u3057\u3002\u540D\u79F0\u306F\u4E0D\u660E`);
      continue;
    }
    if (i2.kind !== "\u8A3A\u7642\u884C\u70BA") {
      out.push(`  ${esc(c)}: \u7A2E\u5225=${i2.kind} \u2192 \u96FB\u5B50\u70B9\u6570\u8868(\u8A3A\u7642\u884C\u70BA)\u306E\u5224\u5B9A\u5BFE\u8C61\u5916`);
      continue;
    }
    if (!i2.found) {
      out.push(`  ${esc(c)}: \u8A3A\u7642\u884C\u70BA\u30DE\u30B9\u30BF\u30FC(${esc(R2.edition)})\u306B\u53CE\u8F09\u306A\u3057 \u2192 \u540D\u79F0\u30FB\u5C5E\u6027\u306F\u4E0D\u660E`);
      continue;
    }
    out.push(`  ${esc(c)}: ${esc(i2.name)} (\u533A\u5206\u756A\u53F7 ${i2.kubun ? refLink(i2.kubun) : "\u4E0D\u660E"})`);
    if (i2.expired) out.push(`      \u203B\u8A3A\u7642\u5E74\u6708\u6642\u70B9\u3067\u5EC3\u6B62\u6E08\u307F(\u5EC3\u6B62\u5E74\u6708\u65E5=${esc(i2.haishi)})`);
    if (i2.nyugai && i2.nyugai !== "0") out.push(`      \u5165\u5916\u9069\u7528\u533A\u5206=${esc(i2.nyugai)}(${esc(NYUGAI[i2.nyugai] ?? "\u4E0D\u660E")})`);
    if (i2.tsusoku && i2.tsusoku !== "0") out.push(`      \u901A\u5247\u5E74\u9F62=${esc(i2.tsusoku)}(\u5024\u306E\u610F\u5473\u306F\u4ED5\u69D8\u8AAC\u660E\u66F8\u53C2\u7167)`);
    if (i2.kagen && i2.kagen !== "00" || i2.jogen && i2.jogen !== "00")
  ...
  if (R2.commentYoken.length) {
    ...
    for (const cy of R2.commentYoken) {
      ...
      for (const b2 of cy.blocks) {
        let head = `[\u5225\u8868${b2.bessyo} \u9805\u756A${b2.koban}]`;
        if (b2.kubun_hyoki) head += ` ${b2.kubun_hyoki}`;
        if (b2.meisho) head += ` ${b2.meisho}`;
        out.push(`    ${esc(head)}`);
        let jiko = (b2.kisai_jiko ?? "").split(/\s+/).join(" ");
        if (jiko.length > 120) jiko = jiko.slice(0, 120) + "\u2026(\u7D9A\u304D\u306F\u5225\u8868\u539F\u6587)";
        out.push(`      \u8A18\u8F09\u4E8B\u9805: ${esc(jiko)}`);
        for (const c of b2.comments)
  ...
  if (R2.shinryokoi.length >= 2) {
    ...
    for (const h2 of R2.hits.filter((h3) => h3.kind === "haihan" || h3.kind === "hokatsu")) {
      out.push(`${esc(h2.title)}: ${esc(h2.detail)}`);
      out.push(`\u6839\u62E0: ${esc(h2.table)} \u8A72\u5F53\u884C:`);
      for (const e of h2.evid) out.push(`  ${esc(e)}`);
      out.push(`\u6B21\u306B\u78BA\u8A8D: \u7559\u610F\u4E8B\u9805\u901A\u77E5\u30FB\u544A\u793A ${h2.refs.map(refLink).join(", ") || "(\u533A\u5206\u756A\u53F7\u306F\u30DE\u30B9\u30BF\u30FC\u4E0A\u4E0D\u660E)"}`, "");
    }
    for (const [a, b2] of R2.notListed) {
      out.push(`[\u53CE\u8F09\u306A\u3057] ${esc(a)} ${esc(R2.infos[a].name ?? a)} \xD7 ${esc(b2)} ${esc(R2.infos[b2].name ?? b2)}: ${esc(NOT_LISTED)}`);
    }
    if (R2.notListed.length) out.push("");
  }
  const nyuinHits = R2.hits.filter((h2) => h2.kind === "nyuin");
  if (nyuinHits.length) {
    out.push("--- \u5165\u9662\u57FA\u672C\u6599(\u5165\u9662\u6599\xD7\u52A0\u7B97) ---");
    for (const h2 of nyuinHits) {
      out.push(`${esc(h2.title)}: ${esc(h2.detail)}`);
      out.push(`\u6839\u62E0: ${esc(h2.table)} \u8A72\u5F53\u884C:`);
      for (const e of h2.evid) out.push(`  ${esc(e)}`);
      out.push("");
  ...
  if (R2.drugs.length || R2.diseases.length) {
    if (!ckEdition) {
      ...
      for (const t of R2.tekiou) {
        ...
        for (const m2 of t.matches) {
          out.push(`    \u25CB \u9069\u5FDC\u306B\u8A72\u5F53: ${esc(m2.code)} ${esc(m2.name ?? "(\u540D\u79F0\u4E0D\u660E)")}`);
          for (const r of m2.rows.slice(0, 3)) {
  ...
  for (const c of R2.codes) {
    const i2 = R2.infos[c];
    const key = `${i2.kubun}|${i2.name}`;
    if (seenKeys.has(key)) continue;
    seenKeys.add(key);
    const label = [i2.kubun, i2.name ?? c].filter(Boolean).join(" ");
    const gigiFiles = {};
    if (i2.kubun) {
      let ref = i2.kubun;
      ...
      if (!hit.length) {
        const m2 = ref.match(/^([A-Z])(\d{3})(.*)$/);
        if (m2 && m2[3] && !m2[3].startsWith("-"))
          hit = await rows("SELECT loc, extra FROM doc_mention WHERE source='gigi' AND ref=?", [m2[1] + m2[2]]);
      }
      for (const h2 of hit) gigiFiles[h2.loc] ??= new Set(h2.extra.split(",").map(Number));
    }
    for (const v2 of nameVariants(i2.name ?? "")) {
      const hit = await rows(
        "SELECT filename, page FROM doc_gigi_page WHERE body LIKE ? ESCAPE '\\'",
        [`%${likeEsc(v2)}%`]
      );
      for (const h2 of hit) (gigiFiles[h2.filename] ??= /* @__PURE__ */ new Set()).add(Number(h2.page));
    }
    ...
    if (gg.length || tt.length) {
      const parts = gg.slice(0, 4).map(([f, p2]) => `${gigiLink(f)}(p.${[...p2].sort((a, b2) => a - b2).slice(0, 4).join(",")})`);
    ...
    const sAll = /* @__PURE__ */ new Map();
    if (i2.kubun) {
      let hit = await rows("SELECT loc, extra FROM doc_mention WHERE source='shinsa' AND ref=?", [i2.kubun]);
      if (!hit.length) {
        const m2 = i2.kubun.match(/^([A-Z])(\d{3})(.*)$/);
        if (m2 && m2[3] && !m2[3].startsWith("-"))
          hit = await rows("SELECT loc, extra FROM doc_mention WHERE source='shinsa' AND ref=?", [m2[1] + m2[2]]);
      }
      for (const h2 of hit) sAll.set(Number(h2.loc), h2.extra);
    }
    for (const v2 of nameVariants(i2.name ?? "")) {
      const hit = await rows(
        "SELECT no, title FROM doc_shinsa WHERE title LIKE ? ESCAPE '\\' OR body LIKE ? ESCAPE '\\'",
        [`%${likeEsc(v2)}%`, `%${likeEsc(v2)}%`]
      );
      for (const h2 of hit) sAll.set(Number(h2.no), h2.title);
    }
    if (sAll.size) {
      const nos = [...sAll.keys()].sort((a, b2) => a - b2);
  ...
  if (gigiLines.length || shinsaLines.length || R2.jireiSummary.length) {
    out.push("--- \u95A2\u9023\u60C5\u5831(\u53C2\u8003\u30FB\u6A5F\u68B0\u7684\u306A\u8A00\u53CA\u7167\u5408) ---");
    if (gigiLines.length) {
      out.push("\u25A0 \u7591\u7FA9\u89E3\u91C8\u4E8B\u52D9\u9023\u7D61\u3067\u306E\u8A00\u53CA(\u533A\u5206\u756A\u53F7\u30FB\u540D\u79F0\u3067\u7167\u5408):");
      out.push(...gigiLines);
    }
    if (shinsaLines.length) {
      out.push("\u25A0 \u5BE9\u67FB\u306E\u4E00\u822C\u7684\u306A\u53D6\u6271\u3044\u3067\u306E\u8A00\u53CA(\u533A\u5206\u756A\u53F7\u30FB\u540D\u79F0\u3067\u7167\u5408):");
      out.push(...shinsaLines);
    }
    if (R2.jireiSummary.length) {
      out.push("\u25A0 \u30B3\u30F3\u30D4\u30E5\u30FC\u30BF\u30C1\u30A7\u30C3\u30AF\u516C\u958B\u4E8B\u4F8B(\u652F\u6255\u57FA\u91D1):");
      for (const j2 of R2.jireiSummary) {
        const samples = j2.samples.map((s) => `${s.jirei_code}(${s.check_kanten})`).join("\u3001");
        out.push(`  ${esc(j2.code)} ${esc(j2.name ?? "")}: ${j2.count}\u4EF6(\u4F8B: ${esc(samples)}) \u2192 <a class="reflink" data-jirei="${esc(j2.code)}">\u4E8B\u4F8B\u672C\u6587\u3092\u8868\u793A</a>`);
      }
    }
...
async function runSearch(query) {
  const q2 = toHw(query.trim());
  const result = { mode: "", rows: [], total: 0 };
  const push = (kind, r) => result.rows.push({ kind, ...r });
  const like = `%${likeEsc(q2)}%`;
  const ed = async (t) => (await rows(`SELECT max(edition) AS e FROM ${t}`))[0]?.e;
  if (/^\d{9}$/.test(q2)) {
    result.mode = "code";
    if (q2[0] === "1") {
      for (const r of await rows(`SELECT shinryokoi_code AS code, shoryaku_kanji_meisho AS name, ${KUBUN_EXPR} AS kubun, shin_matawa_gen_tensu AS value, haishi_ymd FROM master_shinryokoi WHERE edition=? AND shinryokoi_code=?`, [await ed("master_shinryokoi"), q2])) push("\u8A3A\u7642\u884C\u70BA", r);
    } else if (q2[0] === "6") {
      for (const r of await rows("SELECT iyakuhin_code AS code, kanji_meisho AS name, shin_mata_wa_gen_kingaku AS value, haishi_ymd FROM master_iyakuhin WHERE edition=? AND iyakuhin_code=?", [await ed("master_iyakuhin"), q2])) push("\u533B\u85AC\u54C1", r);
    } else if (q2[0] === "7") {
      for (const r of await rows("SELECT tokutei_kizai_code AS code, kanji_meisho AS name, shin_matawa_gen_kingaku AS value, haishi_ymd FROM master_kizai WHERE edition=? AND tokutei_kizai_code=?", [await ed("master_kizai"), q2])) push("\u7279\u5B9A\u5668\u6750", r);
    }
  } else if (/^\d{7}$/.test(q2)) {
    result.mode = "code";
    for (const r of await rows("SELECT shobyomei_code AS code, shobyomei_kihon_meisho AS name, haishi_ymd FROM master_shobyomei WHERE edition=? AND shobyomei_code=?", [await ed("master_shobyomei"), q2])) push("\u50B7\u75C5\u540D", r);
  } else if (/^[A-Za-z][0-9\-]*$/.test(q2)) {
    result.mode = "kubun";
    const all = await rows(`SELECT shinryokoi_code AS code, shoryaku_kanji_meisho AS name, ${KUBUN_EXPR} AS kubun, shin_matawa_gen_tensu AS value, haishi_ymd FROM master_shinryokoi WHERE edition=? AND ${KUBUN_EXPR} LIKE ? ESCAPE '\\' ORDER BY kubun, code`, [await ed("master_shinryokoi"), likeEsc(q2.toUpperCase()) + "%"]);
...
function renderCart() {
  if (!cart.length) {
    $2("selPanel").innerHTML = "";
    return;
  }
  const items = cart.map((it) => `${esc(it.code)} ${esc(it.name ?? "")} <a class="reflink" data-uncart="${esc(it.code)}">[\xD7\u5916\u3059]</a>`).join("<br>");
  $2("selPanel").innerHTML = `<fieldset><legend>\u9078\u629E\u4E2D\u306E\u30B3\u30FC\u30C9(${cart.length}\u4EF6)</legend>${items}
...
function renderSearch(res) {
  if (!res.rows.length) {
    $2("searchResult").innerHTML = "<p>\u8A72\u5F53\u306A\u3057\u3002\u540D\u79F0\u306F\u53CE\u8F09\u8868\u8A18(\u30AB\u30CA\u306F\u534A\u89D2\u30AB\u30CA\u7B49)\u306E\u305F\u3081\u3001\u8868\u8A18\u3092\u5909\u3048\u3066\u518D\u691C\u7D22\u3057\u3066\u307F\u3066\u304F\u3060\u3055\u3044\u3002</p>";
    return;
  }
  ...
  $2("searchResult").innerHTML = `<p>\u30D2\u30C3\u30C8 ${res.total}\u4EF6${res.total > res.rows.length ? `(\u5148\u982D${res.rows.length}\u4EF6\u3092\u8868\u793A)` : ""}</p>
...
${sec[0].body}`);
  ...
  $2("refResult").innerHTML = parts.length ? `<pre>${esc(parts.join("\n\n"))}</pre>` : `<p class="error">${esc(ref)}: \u901A\u77E5\u30FB\u544A\u793A\u306B\u500B\u5225\u8A18\u8F09\u306E\u898B\u51FA\u3057\u304C\u3042\u308A\u307E\u305B\u3093(\u901A\u5247\u30FB\u539F\u672CPDF\u3092\u78BA\u8A8D)</p>`;
  activateTab("ref");
}
async function showGigi(file) {
  ...
  const text = pages.map((p2) => `## p.${p2.page}
${p2.body}`).join("\n\n");
  $2("refResult").innerHTML = `<h3>${esc(file)}</h3><pre>${esc(text)}</pre>`;
  activateTab("ref");
}
async function showShinsa(no) {
  const e = await rows("SELECT no, title, body FROM doc_shinsa WHERE no=?", [String(no)]);
  if (!e.length) return;
  $2("refResult").innerHTML = `<h3>\u5BE9\u67FB\u306E\u4E00\u822C\u7684\u306A\u53D6\u6271\u3044 No.${e[0].no} ${esc(e[0].title)}</h3><pre>${esc(e[0].body)}</pre>`;
  activateTab("ref");
}
async function showJirei(code) {
  ...
  const text = rws.map((r) => `[${r.jirei_code}] \u89B3\u70B9=${r.check_kanten} \u5BFE\u8C61=${r.check_taisho} \u53C2\u7167\u7BC4\u56F2=${r.sansho_hani} \u6839\u62E0=${r.konkyo || "-"} \u516C\u958B=${r.kokai_ymd}
  \u30C1\u30A7\u30C3\u30AF\u5185\u5BB9: ${r.check_naiyo}
  \u30C1\u30A7\u30C3\u30AF\u6839\u62E0: ${r.check_konkyo}`).join("\n\n");
  $2("refResult").innerHTML = `<h3>\u30B3\u30F3\u30D4\u30E5\u30FC\u30BF\u30C1\u30A7\u30C3\u30AF\u516C\u958B\u4E8B\u4F8B ${esc(code)}(${rws.length}\u4EF6)</h3><pre>${esc(text)}</pre>`;
  activateTab("ref");
}
...
async function onRunCheck() {
  const codes = $2("codes").value.trim().split(/[\s,、]+/).filter(Boolean).map(toHw);
  if (!codes.length) {
    $2("checkResult").innerHTML = '<p class="error">\u30B3\u30FC\u30C9\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044</p>';
    return;
  }
...
document.addEventListener("click", async (ev) => {
  ...
  if (t.dataset.ref) {
    await showRef(t.dataset.ref);
    return;
  }
  if (t.dataset.gigi) {
    await showGigi(t.dataset.gigi);
    return;
  }
  if (t.dataset.shinsa) {
    await showShinsa(t.dataset.shinsa);
    return;
  }
  if (t.dataset.jirei) {
    await showJirei(t.dataset.jirei);
    return;
  }
  if (t.dataset.cart) {
  ...
  if (t.id === "toCheck") {
    $2("codes").value = cart.map((c) => c.code).join(" ");
    activateTab("check");
    return;
  }
  if (t.id === "clearCart") {
...
$2("runCheck").addEventListener("click", onRunCheck);
$2("runSearch").addEventListener("click", async () => {
  $2("searchResult").innerHTML = "<p>\u691C\u7D22\u4E2D\u2026</p>";
  try {
    renderSearch(await runSearch($2("searchQ").value));
  } catch (e) {
    $2("searchResult").innerHTML = `<p class="error">\u30A8\u30E9\u30FC: ${esc(e.message)}</p>`;
  }
});
$2("searchQ").addEventListener("keydown", (e) => {
  if (e.key === "Enter") $2("runSearch").click();
});
$2("runRef").addEventListener("click", () => showRef($2("refQ").value));
$2("refQ").addEventListener("keydown", (e) => {
  if (e.key === "Enter") $2("runRef").click();
});
